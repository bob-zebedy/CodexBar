...
- 工具: `tool_name`
- 模型: `model`
- 推理强度来源: `transcript_path`；只在 `UserPromptSubmit` / `PermissionRequest` 时从对应 rollout 尾部最多 512 KB 查找同 turn 的 `turn_context.effort`，写为 `effort`
...
- 初次从文件末尾最多读取 512 KB，之后每 1 秒按 byte offset 增量读取，保留半行并检测截断或替换。初始窗口未覆盖完整历史且活跃 turn 的 effort 仍缺失时，会在任务开始至少 2 秒后按精确 turn 从文件尾部最多 8 MB 一次性回查 `turn_context.effort`；每轮 lifecycle 查询最多回查一个 turn，读取失败至少间隔 10 秒重试，成功读取但未找到则不再回查。只解出顶层 `type`、`payload.type`、`payload.turn_id`、`started_at`、`completed_at`、`duration_ms`、`turn_context.approvals_reviewer` 和 `turn_context.effort`；不提取、保存或展示提示词、回复、推理内容、工具内容、审批内容或 token 数据。
...
- lifecycle 查询读到 `turn_context.effort` 时会回填当前 turn 的推理强度；Hook 与 rollout 值一致时保持原值，缺失时沿用已有值，若同一 turn 出现冲突则标记为 `mixed`，不会静默覆盖。推理强度会随任务进入完成或终止记录。
...
- `SubagentStart` / `SubagentStop` 的 `turn_id` 属于子 Agent 自己，不等于父任务 turn；子 Agent 活跃数通过共享 `session_id` 归属到该 session 下唯一的当前顶层任务。关联成功后，早于顶层任务可信开始时间的事件会被视为上一 turn 的迟到事件并忽略。monitor 以 `agent_id` 保存每个子 Agent 最新的开始/结束状态，重复事件幂等，旧时间戳不会覆盖新状态；遇到缺失 `agent_id`、同 session 存在多个活动或等待终态确认的顶层任务、无法关联任务，或没有已知开始状态的结束事件后，将当前任务标记为不可可靠统计并隐藏数量。数量可靠且大于 0 时才显示在主面板当前活动卡片，任务中心不显示；任务结束或新 turn 替换旧 turn 时随内存任务状态一并清除。Hook 整体缺失结束事件无法被主动识别，因此该值仍是基于完整 Hook 序列的最佳努力实时状态。
...
设置页「重建数据」使用两次点击选择最近 210 天内的开始和结束日期，选择期间可以跨月；连续点击同一天两次时形成单日范围。保留窗口内的日期均可选择，非空本机原始事件日期以圆点标记，未来日期和保留窗口外日期不可选。选择范围完成后，行内消息显示完整日历范围包含的天数；真正提交时只取范围内仍存在且文件非空的 `events/YYYY-MM-DD.jsonl` 日期批量重建，没有数据的日期不会生成空聚合。例如 1–15 号中只有 1、2、3、4、9、10 号有数据时，确认弹窗和重建任务均按 6 天处理。每次打开设置窗口都会清除上次选择和结果消息。

用户确认后，`WorkflowSyncScheduler` 将整批重建与普通维护、CloudKit 同步串行调度；已有任务先完成，重建期间产生的自动维护或同步请求合并排队，并在重建结束后继续处理。`WorkflowService` 依次从每个待重建文件第一行开始重新聚合，为对应日期生成新的 `sourceGeneration`，并原子更新 `daily.jsonl`；原始事件文件不会被修改。重建期间允许 Hook 继续追加，每个任务只处理开始时捕获的稳定文件边界，随后新增内容仍由 pending 增量维护补齐；如果任一文件被截断、替换或原位置内容再次变化，本批重建失败并保留 dirty 状态供重试。已成功完成的前序日期仍会保留其本地结果和待云端替换状态。

重建表示用户明确选择「以本机这些日期现存原始事件为准」。因此它与自动检测到的来源不明替换不同：同步状态会持久化各日期的权威替换请求，展示时先忽略当前设备对应日期的旧云端缓存；同步可用后，全量确认并删除当前设备对应日期的所有旧 generation，再以本地新 generation 上传重建聚合。上传确认后，本机和远端使用同一个 generation。范围外日期、没有本机数据的日期、其他设备贡献以及原始事件不受影响；同步暂不可用或失败时替换请求保留，后续同步自动重试。
...
`effort` 用于本机实时任务展示，`agent` 用于子 Agent 关联；两者都不参与每日计数或 `daily.jsonl` schema，也不会进入 CloudKit 同步副本。`agent` 不直接展示给用户；原始值只随对应 Hook 行保存在本机事件文件中。
