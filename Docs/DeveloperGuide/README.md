...
| [app-server 数据链路](app-server.md) | CLI 定位、JSON-RPC 会话、账户、额度、用量刷新和自动重置状态机 |
...
| 修改自动重置调度或唤醒计划 | [app-server 数据链路](app-server.md) | [防睡眠系统](sleep-prevention.md)、[数据与隐私边界](data-and-privacy.md) |
...
- CodexBarHelper 只负责固定的睡眠控制和自动重置唤醒计划，不增加网络、任意命令执行或额外文件访问
