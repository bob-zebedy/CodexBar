...
| 自动重置 | app-server mutation 结果 | 只有明确结果才能说明本机自动操作发生过 |
| 任务完成 | monitor 转场 | 历史 terminal 快照不能补发 |
| 等待批准 | monitor 转场加当前快照 | 先通知新等待，再撤回过时等待 |
| 异常会话保护 | 达到静默阈值且仍有效的保护候选 | 提交通知与隐藏前分别校验候选 |
...
一次自动重置可能同时满足两者，所以同一台设备可以先后收到两条通知。不能因为内容接近就合并：前者证明自动化动作，后者是额度生命周期通知，来源和开关都不同。
...
非匿名运行任务静默达到阈值时，Activity Protection 先登记保护记录，再并行启动通知提交和 3 秒宽限。通知处理返回或宽限到期后，monitor 重新校验候选，仍有效时才隐藏任务。通知失败也会继续完成保护。
