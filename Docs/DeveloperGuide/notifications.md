...
- `ResetCreditAutomationController` 已确认的消费结果或需要告知用户的失败状态
...
| 自动使用重置 | app-server mutation 结果 | 只有明确结果才能说明本机自动操作发生过 |
...
| 自动使用重置 | 已确认消费结果或需要告知用户的失败状态 | 自动使用开关、独立通知子开关和通知总开关 |
...
自动使用和额度重置是两个独立事件，因此保留两条通知：

- “自动使用重置”说明 CodexBar 的自动操作收到 `reset` 或 `alreadyRedeemed`
- “额度已重置”说明普通额度快照先观察到消耗，随后观察到归零

一次自动使用可能同时满足两者，所以同一台设备可以先后收到两条通知。不能因为内容接近就合并：前者证明自动化动作，后者是额度生命周期通知，来源和开关都不同。
...
成功和失败通知共用“自动使用重置通知”子开关及声音，默认开启并使用系统默认声音。自动使用功能关闭时设置行置灰，但不覆盖已经保存的通知和声音选择；重新开启后恢复原值。
...
- 低额度只在向下跨越阈值时通知
- 初次加载 100% 额度不误报重置
- 自动使用明确返回 `reset` 时发送“自动使用重置”，后续额度归零时仍可独立发送“额度已重置”
- 自动使用明确返回 `alreadyRedeemed` 时按成功通知并停止重试
- 只看到目标凭证消失时不发送自动使用通知
- 网络重试期间不发送失败通知，同一凭证的同一失败原因最多通知一次
- 自动使用关闭时通知子项置灰，重新开启后恢复原来的开关和音效
- 成功和失败通知使用“自动使用重置通知”所选音效，选择静音时两者都无声
- 完成时长低于和高于阈值时行为正确
