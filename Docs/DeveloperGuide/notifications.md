...
- 通知服务还维护自己的已发送 key
- `Stop` 保留正在收尾的任务；rollout 确认完成后，结束时间不早于恢复界限且距当前不超过 10 秒时发布完成转场
- bootstrap、恢复对账和 Hook 数据源不健康期间，完成记录静默更新
...
- 自动审批不通知
- 同一任务连续等待期间只通知一次，其他 Agent 的进展不结束该等待；审批归属规则见 [等待批准的两阶段确认](activity-monitor.md#等待批准的两阶段确认)
- 任务离开等待状态后从相关集合移除
- bootstrap、恢复对账和 Hook 数据源不健康期间不发布等待转场
- Hook 直接确认和 rollout 补齐 reviewer 后确认均要求任务进入等待的时间不早于恢复界限且距当前不超过 10 秒
...
App idle assertion 保留到通知提交结束，随后释放，由 macOS 按系统电源策略决定睡眠时机。
