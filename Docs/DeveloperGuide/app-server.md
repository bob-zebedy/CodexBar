...
只有实际版本满足全局门槛，并且 handshake 与首次账户读取都成功，connection 才进入 service 状态。明确低于 `0.145.0` 时返回版本不支持；版本缺失或无法解析时返回无效响应，不能作为 Hook 自动清理的低版本依据。两种情况都会关闭半初始化 session，不继续发送账户请求。
