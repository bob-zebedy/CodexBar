...
设置从关闭切换为开启且 Hook 可用时，`TaskGlowSettings.previewRequests` 请求一次使用当前外观设置的运行预览，完成一个往返周期后恢复实际状态，标准速度下周期为 2.7 秒。启动和设置刷新只恢复开关值，不触发预览。预览覆盖期间继续接收真实任务更新，不产生任务事件。

播放和收尾期间忽略新的预览请求。关闭开关通过光带已有的退场流程快速收回到中心并淡出；所有屏幕实际收尾完成后释放预览占用。Hook 不可用、系统睡眠、屏幕休眠或会话不活跃时取消预览。
...
- 开启流光按当前颜色、速度和亮度播放一次预览，关闭时收回并淡出，收尾期间不重播，收尾后可再次开启预览；预览期间任务状态变化后正确接续实际状态
