...
创建 settings 和数据服务
  -> 建立 ViewModel
  -> 启动 activity monitor
  -> 连接 notification 与 keep-alive 副作用
...
终止时顺序反转, 但防睡眠是例外. AppDelegate 先异步确认 helper lease 已释放, 成功后才允许进程终止. 详细事务见 [防睡眠系统](sleep-prevention.md)
...
通知和防睡眠的二级设置面板按需创建. 控制器一旦创建就会长期订阅内容高度变化, 如果 App 启动时预建所有面板, 从未使用的 UI 也会一直参与更新
