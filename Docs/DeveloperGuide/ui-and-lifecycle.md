...
| 场景 | 展示规则 |
| --- | --- |
| 存在活跃任务 | 橙色等待批准优先于青色运行 |
| 其他任务仍活跃时确认结束事件 | 最新事件覆盖活动状态 3 秒，最后 0.5 秒淡出；新事件替换提示并重新计时 |
| 无活跃任务 | 快照中的最新终态显示到结束后 10 秒，最后 1 秒淡出 |

完成使用绿色，终止使用红色；结束时间相同优先终止。所有计时包含进场动画，到期后按最新快照恢复展示。

设置从关闭切换为开启且 Hook 可用时，`TaskGlowSettings.previewRequests` 请求一次青色运行预览，完成一个 2.7 秒往返周期后恢复实际状态。启动和设置刷新只恢复开关值，不触发预览。预览覆盖期间继续接收真实任务更新，不产生任务事件。

播放和收尾期间忽略新的预览请求。关闭开关通过光带已有的退场流程快速收回到中心并淡出；所有屏幕实际收尾完成后释放预览占用。Hook 不可用、系统睡眠、屏幕休眠或会话不活跃时取消预览。

光带使用不激活、穿透鼠标的 `NSPanel`，跨 Space 和全屏显示。系统睡眠、屏幕休眠或用户会话不活跃时移除窗口，恢复后按当前时间接续有效状态。屏幕配置变化时重建窗口，各显示器共享运动时钟。
...
- 多任务运行或等待批准期间，完成和终止触发 3 秒流光提示；连续结束及全部任务结束时颜色与到期时间正确
- 开启流光播放一次预览，关闭时收回并淡出，收尾期间不重播，收尾后可再次开启预览；预览期间任务状态变化后正确接续实际状态
