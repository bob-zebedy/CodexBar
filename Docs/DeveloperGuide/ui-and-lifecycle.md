...
`TaskGlowController` 在 `TaskGlowSettings.isEnabled` 和 Hook 的 `isOperable` 均为 `true` 时显示任务状态，与主面板动画开关独立。输入来自 `CodexActivityMonitor.presentationPublisher`，包含当前快照与本轮结束事件，见[展示更新](activity-monitor.md#展示更新)

| 场景 | 展示规则 |
| --- | --- |
| 存在活跃任务 | 等待批准优先于运行 |
| 其他任务仍活跃时确认结束事件 | 最新事件覆盖活动状态 3 秒，最后 0.5 秒淡出；新事件替换提示并重新计时 |
| 无活跃任务 | 快照中的最新终态按设置的结束时长显示，默认结束后 10 秒到期，最后 1 秒淡出 |

颜色、速度、亮度和结束时长由 `TaskGlowSettings.appearance` 提供。默认运行使用青色，等待批准使用橙色，完成使用绿色，终止使用红色；结束时间相同优先终止。所有计时包含进场动画，到期后按最新快照恢复展示。

设置从关闭切换为开启且 Hook 可用时，`TaskGlowSettings.previewRequests` 请求一次使用当前外观设置的运行预览，完成一个往返周期后恢复实际状态，标准速度下周期为 2.7 秒。启动和设置刷新只恢复开关值，不触发预览。

双击设置面板中的颜色块请求对应状态的预览：运行播放一个往返周期，等待批准展开后播放两次呼吸，完成和终止显示 3 秒，最后 0.5 秒淡出。颜色预览可以替换正在播放的预览；收起设置子面板只结束颜色预览。开启预览在已有预览或窗口收尾期间忽略重复请求。

预览覆盖期间继续接收真实任务更新，不产生任务事件。真实结束提示暂停计时，恢复时按最新快照展示：已有提示继续剩余时长，暂停期间产生的提示保留完整时长。预览切换时各屏幕共用开始时刻，旧光带收回时间不占用目标效果的展示时长。

关闭开关取消预览，光带快速收回到中心并淡出，所有屏幕实际收尾完成后移除窗口。Hook 不可用、系统睡眠、屏幕休眠或会话不活跃时取消预览；睡眠期间真实结束提示按墙上时间到期。

光带使用不激活、穿透鼠标的 `NSPanel`，跨 Space 和全屏显示。系统睡眠、屏幕休眠或用户会话不活跃时移除窗口，恢复后按当前时间接续有效状态。屏幕配置变化时重建窗口，各显示器共享运动时钟。运动时钟根据分段时长和速度计算周期，改速时保留当前进度。
...
```text
账户异常 > 等待批准 > 运行中 > 最近 10 秒内的完成或终止 > 空闲
```

`CodexActivitySnapshot.statusItemActivity(at:)` 在没有活跃任务时按结束时间选择最新终态。`StatusItemController` 按结束时间加 10 秒安排到期任务，到期后恢复普通人形；系统唤醒时按当前时间重算，状态变化和卸载时取消旧任务。
...
- `StatusItemIconView` 按符号名称和屏幕倍率缓存预渲染图片，缩放时复用图片，避免带徽章符号实时渲染时的卡顿
- 符号名称变化时，新旧图片在 0.2 秒内交叉淡入淡出，并在 75% 与完整大小之间缩放；只切换额度显隐时不触发图片替换
...
主面板布局、流光、通知、自动重置和防睡眠的二级设置面板在首次打开时创建，之后复用内容及必要的高度变化订阅。

这些设置子面板使用可获得键盘焦点的 `KeyableBorderlessPanel`，展开时保留主设置窗口的焦点，点击输入框后进入编辑。收起面板前结束原生编辑，颜色输入在结束编辑回调中完成校验并同步显示。主面板的热力图、Reset Credits 和活动中心详情使用不会激活的 `NonactivatingSidePanel`。收起设置子面板时，`SidePanelSupport.orderOut` 只在该子面板仍是 key window 时恢复父窗口焦点；焦点已转移到其他窗口时不恢复父窗口焦点。
...
- 主面板布局入口始终可用
- 流光要求 `TaskGlowSettings.isEnabled`、Hook 的 `isOperable` 为 `true`，且 Hook 未在更新
...
`SettingsWindowController` 持有这一窗口组唯一的 `UndoManager`。设置主窗口通过 `AuxiliaryHostingWindow` 暴露它，各设置子面板展示时从父窗口取得同一实例，因此焦点位于设置主窗口或任一子面板时，`⌘Z` 和 `⌘⇧Z` 都作用于同一份布局历史。Hook 状态变化引起的任务中心自动显隐不进入用户撤销历史。
...
app-server 状态默认每 60 秒检查刷新。主面板的 0.24 秒淡入完成且面板仍可见时，调用 `refreshIfNeeded`；倒计时起点为空或距上次刷新结果提交超过 60 秒时才发起请求。成功和失败的结果提交都会重置倒计时。

主面板显示刷新倒计时，双击账户图标可立即手动刷新。

普通刷新在刷新或重连进行中被忽略。需要在当前请求后补刷的操作通过 `refreshAfterCurrent` 保存一个待执行触发，当前请求结束后再执行。

淡入完成回调依次触发 Hook 配置对账、Hook 开启时的本地统计刷新检查，以及账户、额度和用量刷新检查。面板在淡入完成前关闭时，取消完成任务及其尚未触发的刷新。
...
- 多任务运行或等待批准期间，完成和终止触发 3 秒流光提示；颜色跟随配置，全部任务结束后按所选结束时长到期
- 开启流光按当前颜色、速度和亮度播放一次预览，关闭时收回并淡出，收尾期间不重播开启预览，收尾后可再次开启预览；双击颜色块可连续替换预览，收起子面板结束颜色预览；恢复时已有结束提示继续剩余时长，预览期间产生的提示按最新状态显示并保留完整时长
- 历史重读及唤醒对账不补播旧终态短提示，恢复后的真实结束仍有提示；多显示器和屏幕配置变化后运动与到期时间一致
- 菜单栏符号随任务状态切换，最新终态在结束 10 秒后恢复空闲，跨睡眠唤醒后按当前时间显示
- 普通人物与四种任务徽章之间切换时过渡完整，快速连续切换后显示最新状态；带徽章时反复切换额度显隐仍保持流畅，首次显示和唤醒重算不播放过渡
...
- 流光子面板展开时颜色输入框不自动聚焦；六位输入限制、粘贴过滤和大写转换正确；无效颜色在回车、失焦及收起时恢复默认，显示与实际颜色一致
