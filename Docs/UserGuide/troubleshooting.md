...
| 提示 | 含义 | 建议处理 |
| --- | --- | --- |
| 需要更高 Codex 版本 | 当前 app-server 版本不足 | 更新 Codex 后在关于页面点击“重新建立连接” |
...
3. 当前 app-server 是否返回了具体的可用留存重置和过期时间
...
如果关于页面显示新安装版本但 `当前使用` 仍是旧版本，点击“重新建立连接”。也可以等待当前连接满 1 小时后的下一次请求自动重建。
