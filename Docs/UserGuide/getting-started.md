...
5. 如果需要实时任务、通知或防睡眠，在 `设置 > 高级` 中开启 CodexBar Hook
...
| 点击活动卡片 | 打开并发任务中心 |
| 点击手动重置次数 | 查看各批次过期时间 |
| `设置 > 高级 > 自动使用临期重置` | 配置是否自动使用临期重置以及过期前多久使用 |
...
| 自动使用临期重置 | 不需要 | 用户主动开启；默认提前 2 小时，本机睡眠时不会主动唤醒 |
| 任务类通知 | 需要 | Hook 已通过校验且通知已开启 |
| 防止系统睡眠 | 需要 | CodexBarHelper 已注册并获得系统批准 |
