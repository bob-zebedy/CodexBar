...
- 通过菜单栏状态点、活动卡片和并发任务中心实时查看 Codex 正在运行、等待批准及刚完成的任务，并展示当前推理强度；可可靠统计且存在活跃子 Agent 时，活动卡片还会显示其数量
...
菜单面板账号卡片下方默认显示活动卡片，汇总当前项目、模型、推理强度、工具、耗时和其他并发任务数量；可可靠统计且存在活跃子 Agent 时还会显示其数量。可在设置中关闭「主面板任务中心」来隐藏活动卡片及任务中心入口；该选项依赖 Codex Hook，Hook 关闭时显示为关闭并置灰，但会保留原偏好。隐藏只影响主面板展示，不停止 Hook 记录、菜单栏任务状态点、任务通知或触觉反馈。模型、推理强度与项目等同一行信息统一使用 `•` 连接。主面板快照不可用时，活动卡片显示「暂无数据」且不开放任务中心；快照可用并存在活动任务、最近完成或最近终止记录时，点击卡片会打开并发任务中心，按等待确认、运行中、最近完成和最近终止分组展示全部任务，其中模型后的推理强度同样使用 `•` 连接。只有终止历史时，卡片显示最近一项灰色终止状态。面板优先在菜单右侧展开，空间不足时自动切换到左侧，并优先与活动卡片顶边对齐；暂时无法取得有效位置时会等待布局更新后重试，仍然失败才在主面板旁垂直居中。绿色完成状态保留 30 秒，最近完成和最近终止记录保留 10 分钟；终止记录不触发完成提醒。

### 额度与重置时间一眼可见

额度区用分段电量条展示不同时间窗口的剩余额度，并显示剩余百分比和重置时间。存在可用手动重置机会时，主 limit 标题右侧会显示 `重置次数: N`，点击后通过侧边详情面板按过期时间展示各重置机会的数量；未来 7 天内过期的次数显示为橙色，少于 24 小时过期的次数显示为红色。
