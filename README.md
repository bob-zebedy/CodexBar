...
其中实时任务, 热力图面板中的详细统计数据与防睡眠功能依赖 CodexBar Hook, 开启方式见[关于 Hook](#关于-hook)
...
需要长时间运行 Codex 任务但是怕系统自动睡眠?

- 仅在任务运行期间防止系统睡眠, 任务结束恢复系统睡眠设置
- 可开启等待批准时保持, 任务等待批准期间继续防止系统睡眠
- 可开启保持屏幕常亮, 防睡眠期间同时阻止屏幕保护与自动锁屏
- 可设置最长防睡眠时间 1, 2, 4, 8, 12, 24 小时或无限制
- 可开启低电量保护, 使用电池且电量跌破阈值时恢复系统睡眠, 接通电源后自动恢复防睡眠
- 防睡眠状态可在主面板的任务卡片右侧显示一个咖啡杯标记

### 通知与提醒

- 本地通知: 额度预警, 额度重置, 任务等待批准, 任务完成, 手动重置即将过期, 低电量保护生效, 防睡眠阈值
- 通知音效独立设置
- 任务状态变化可设置触摸板震动反馈通知
...
> [!WARNING]
> 为了避免异常任务导致系统长时间处于禁止睡眠状态, 建议设置最长防睡眠时间
