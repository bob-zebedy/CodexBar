...
- 通过菜单栏状态点和面板活动卡片实时查看 Codex 正在运行、等待批准及刚完成的任务
...
通知子面板中的「任务触觉反馈」默认开启；当任务进入等待批准或确认结束时，CodexBar 会每 150 ms 请求一次层级变化触觉反馈，连续 10 次。该功能服从 App 内「系统通知」总开关，但不依赖系统通知授权或长任务时长阈值；是否实际反馈及震感强弱由当前输入设备、辅助功能和系统偏好决定。
