...
如果在设置中开启「菜单栏额度指示」，菜单栏图标左侧会以一条细竖向进度条显示当前账号 Codex 额度中所选窗口的剩余比例，未开启时菜单栏图标保持纯图标显示。

### 额度与重置时间一眼可见

额度区用分段电量条展示不同时间窗口的剩余额度，并显示剩余百分比和重置时间。存在可用手动重置机会时，鼠标悬停在 `重置次数: N` 上会按 `过期: yyyy-MM-dd HH:mm:ss • 可用: N` 显示各重置机会的过期时间和数量。
