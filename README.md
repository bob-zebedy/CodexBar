...
其中实时任务, 热力图面板中的详细统计数据与阻止系统休眠功能依赖 CodexBar Hook, 开启方式见[关于 Hook](#关于-hook)
...
- 仅在任务运行期间阻止系统休眠, 任务结束恢复系统休眠设置
- 可设置最长阻止休眠时间 1, 2, 4, 8, 12, 24 小时或无限制
- 阻止休眠状态可在主面板的任务卡片右侧显示一个咖啡杯标记

### 通知与提醒

- 本地通知: 额度预警, 额度重置, 任务等待批准, 任务完成, 手动重置即将过期
- 通知音效独立设置
- 任务状态变化可设置触摸板震动反馈通知
...
> [!WARNING]
> 为了避免异常任务导致系统长时间处于阻止休眠的状态, 建议设置最长阻止休眠时间
...
| 查询额度重置机会的过期时间     | 请求 OpenAI 接口获取数据                        |
