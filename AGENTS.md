...
本文件是 Codex 在本仓库工作的持久指令。根目录范围内均适用；如果子目录未来出现更近的 `AGENTS.md`, 以更近文件为准。内容按 OpenAI Prompt Guidance(GPT-5.5) 的思路组织: 先定义可验证目标、成功标准、工具与上下文策略, 再列出 CodexBar 的代码事实和实现边界。
