...
def interval_chart(
    ...
    for label, entries in rows:
        ...
        description = (
            f"首段 {first:.1f} → 尾段 {last:.1f} MiB · 变化 {last - first:+.1f} MiB"
            if paired
...
def write_report(data: Report, destination: Path) -> None:
    ...
    hangs = (
        sum(p["potential-hangs"]["count"] for p in hang_phases) if hang_phases else None
    )
    state = {
        "complete": "采集完成",
        "incomplete": "采集不完整",
    ...
    for index, phase in enumerate(phases):
        ...
        matrix_rows.append(
            [
                ...
                "有效" if phase in valid else "部分数据",
            ]
        )
        phase_buttons.append(
            f'<button type="button" class="phase-button {"ok" if phase in valid else "warn"}" aria-controls="phase-{index}"><span>{escape(phase["id"])}</span><b>{number(phase.get("recorded_s"), 1)} s</b></button>'
        )
    matrix = table(
        [
            "阶段",
    ...
    def resource_groups(
        ...
        for phase, r in resources:
            ...
            for index, (label, key) in enumerate(specification):
                value = r.get(key)
                entries.append(
                    (label, value / scale if value is not None else None, COLORS[index])
                )
            groups.append((phase["id"], entries))
        return groups

    comparisons = [
        panel("平均 CPU", bars(resource_groups([("均值", "cpu_mean")]), "%")),
        panel(
            "CPU P95 与峰值",
    ...
    comparisons.append(
        panel(
            "footprint 首尾变化",
            interval_chart(
                [(name, [t["head_median"], t["tail_median"]]) for name, t in trends],
                paired=True,
            ),
    ...
    comparisons.append(
        panel(
            "footprint 变化斜率",
            ranked(
                [(name, t["slope_mib_min"]) for name, t in trends], "MiB/min", digits=2
            ),
            "按阶段样本作最小二乘拟合",
        )
    )
    comparisons.append(
        panel(
            "footprint 首尾差值",
            ranked([(name, t["delta_mib"]) for name, t in trends], "MiB", digits=2),
            "尾段窗口中位数减去首段窗口中位数",
        )
    )
    io_charts = [
        panel(
            "Idle Wake Ups 速率",
            bars(resource_groups([("均值", "wakeups_per_s")]), "次/s"),
        ),
        panel(
            "磁盘读取与写入",
            bars(
                resource_groups(
                    [("读取", "read_bytes"), ("写入", "written_bytes")], 1024
                ),
                "KiB",
                1,
            ),
        ...
        panel(
            ...
            bars(
                resource_groups(
                    [("读取", "read_bytes_per_s"), ("写入", "written_bytes_per_s")],
                    1024,
                ),
                "KiB/s",
            ),
    ...
    for index, phase in enumerate(phases):
        r = phase.get("resources", {})
        content = f'<article class="phase" id="phase-{index}" tabindex="-1"><div class="section-head"><div><span class="eyebrow">{escape(phase["template"])}</span><h3>{escape(phase["id"])}</h3></div><span class="badge {"ok" if phase in valid else "warn"}">{"已采集" if phase in valid else "部分数据"}</span></div>'
        content += f'<p class="muted">{escape(phase.get("trace_started", phase.get("started", "N/A")))} · {number(phase.get("recorded_s"), 2)} s</p>'
        if r:
            metrics = [
                ("CPU 均值", number(r.get("cpu_mean"), 2, "%")),
                (
                    "CPU P50 / P95",
                    number(r.get("cpu_p50"), 2)
                    + " / "
                    + number(r.get("cpu_p95"), 2, "%"),
                ),
                ("唤醒次数", number(r.get("wakeups"), 0)),
                ("资源样本", number(r.get("samples"), 0)),
                (
                    "CPU 采样覆盖",
                    number(
                        coverage * 100
        ...
        if profile:
            ...
            hot += '<div class="two">' + panel(
                ...
                ranked(
                    [
                        (f"{t}–{t + 1} s", value)
                        for t, value in profile["busiest_seconds"]
                    ]
                ),
            )
            hot += (
        ...
        record_rows.append(
            [
                phase["id"],
                phase["template"],
                number(phase.get("recorded_s"), 2, " s"),
                r.get("samples", "N/A"),
    ...
    events_chart = bars(
        [
            (
                p["id"],
                [
                    ("Hangs", p.get("potential-hangs", {}).get("count"), COLORS[0]),
                    ("Hang risks", p.get("hang-risks", {}).get("count"), COLORS[2]),
                ],
            )
            for p in phases
        ],
    ...
    metadata: list[list[object]] = [
        ...
        ["电源", env.get("power", "N/A")],
        ["工作负载", data.get("config", {}).get("workload", "N/A")],
    ]
    fingerprint = [
        ["App 文件指纹", target.get("binary_sha256", "N/A")],
        [
            "Mach-O UUID",
            "; ".join(f"{arch}: {uuid}" for uuid, arch in target.get("uuids", [])),
        ],
    ]
    load_groups = [
        (
            label,
            [
                ("开始", env.get("load_before", [None] * 3)[i], COLORS[0]),
    ...
    if comparison:
        ...
        if comparison.get("deltas"):
            optional += '<div class="two">' + panel(
                "平均 CPU 差值",
                ranked(
                    [(r["phase"], r["cpu_pp"]) for r in comparison["deltas"]],
                    "百分点",
                    digits=2,
                ),
            )
            ...
            optional += table(
                ["比较状态", "字段"],
                [["条件不匹配", ", ".join(comparison.get("differences", []))]],
            )
        optional += "</section>"
    ...
    payload = (
        json.dumps(portable, ensure_ascii=False, allow_nan=False)
        .replace("&", "\\u0026")
        .replace("<", "\\u003c")
        .replace(">", "\\u003e")
    )
    ...
    replacements = {
        "TITLE": escape(target.get("name", "App")) + " · 性能数据",
        "APP": escape(target.get("name", "App")),
        "STATE": state,
        "STATE_TONE": tone,
        ...
        "HANGS": str(hangs) if hangs is not None else "N/A",
        "EXTRA_KPIS": extra_kpis_html,
        "MATRIX": matrix,
        ...
        "IO_CHARTS": "".join(io_charts),
        "PHASES": "".join(phase_html),
        "HOT_OVERVIEW": hot_overview,
        "HOTSPOTS": "".join(hotspot_html),
        "EVENTS": panel("Hangs / Hang risks", events_chart) + panel("热状态", thermal),
        "EVENT_DETAILS": table(["阶段", "事件", "记录"], event_rows)
        if event_rows
        else "",
        "RECORDS": table(
            ["阶段", "模板", "记录时长", "样本数", "最大间隔", "CPU 采样覆盖", "状态"],
            record_rows,
        ),
        "METADATA": table(["项目", "值"], metadata),
        "FINGERPRINT": table(["字段", "值"], fingerprint),
        "LOAD": bars(load_groups, "", 2),
        "OPTIONAL": optional,
        "PAYLOAD": payload,
    }
    template = Path(__file__).with_name("performance.html").read_text()
