...
应用是 `LSUIElement` 菜单栏程序（无 Dock 图标、无主窗口），入口 `CodexBarApp` 在启动时先分流：若带 `--hook-event` 参数则走 `WorkflowHookEventRecorder` 记录一条 Hook 事件并立即退出，否则正常起 UI。
...
- 错误处理哲学：UI 只暴露 `notLoggedIn` / `initializationFailed` 两类特殊状态；所有具体请求/启动/超时/解析错误进 `RequestLogStore.shared`（容量 500、单条 4000 字符），不直接展示子进程 stderr。
